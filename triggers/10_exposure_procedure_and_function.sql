...
CREATE OR REPLACE PROCEDURE set_limit_used_of_user(varuser_id uuid)
LANGUAGE plpgsql
AS $a$
BEGIN
WITH market_profit AS
    ...
    (SELECT
	  mp.market_id
	  ,min(mp.runner_profit) as limit_use
...
-- Function: Returns per-runner profit/loss for each non-fancy market the user has active bets in.
-- Pass varmarket_id = 0 or NULL to get all markets.
-- Only works for market_type <> 4 (odds, bookmaker, etc.)
...
-- Function: Returns per-market worst-case profit/loss for fancy/session markets (market_type = 4).
-- Pass varmarket_id = 0 or NULL to get all fancy markets.
-- Returns min(runner_profit) per market — the worst-case exposure across all run scenarios.

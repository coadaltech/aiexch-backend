...
-- CREATE OR REPLACE FUNCTION public.trg_voucher_detail_ledger_after_`insert_update()
--     RETURNS trigger
--     LANGUAGE 'plpgsql'
-- AS $BODY$
-- BEGIN
...
BEGIN
    UPDATE ledger_limit
       SET user_balance = user_balance + (CASE WHEN NEW.record_status <> 1 THEN (CASE WHEN NEW.voucher_type <> 2 THEN (CASE WHEN NEW.dr_cr=1 THEN NEW.amount ELSE -NEW.amount END) ELSE 0 END) ELSE 0 END)
           ,user_limit   = user_limit + (CASE WHEN NEW.record_status <> 1 THEN (CASE WHEN NEW.voucher_type = 2 THEN (CASE WHEN NEW.dr_cr=1 THEN NEW.amount ELSE -NEW.amount END) ELSE 0 END) ELSE 0 END)
           ,final_limit  = final_limit + (CASE WHEN NEW.record_status <> 1 THEN (CASE WHEN NEW.dr_cr = 1 THEN NEW.amount ELSE -NEW.amount END) ELSE 0 END) 
     WHERE user_id = NEW.user_id	 
	 ;
  RETURN NEW;
END;
...
BEGIN
    UPDATE ledger_limit
       SET user_balance = user_balance - (CASE WHEN OLD.record_status <> 1 THEN (CASE WHEN OLD.voucher_type <> 2 THEN (CASE WHEN OLD.dr_cr=1 THEN OLD.amount ELSE -OLD.amount END) ELSE 0 END) ELSE 0 END)
           ,user_limit   = user_limit - (CASE WHEN OLD.record_status <> 1 THEN (CASE WHEN OLD.voucher_type = 2 THEN (CASE WHEN OLD.dr_cr=1 THEN OLD.amount ELSE -OLD.amount END) ELSE 0 END) ELSE 0 END)
           ,final_limit  = final_limit - (CASE WHEN OLD.record_status <> 1 THEN (CASE WHEN OLD.dr_cr = 1 THEN OLD.amount ELSE -OLD.amount END) ELSE 0 END) 
     WHERE user_id = OLD.user_id	 
	 ;
  RETURN NEW;
END;

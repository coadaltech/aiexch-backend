...
// ── User Notifications (per-user TARGETED, e.g. "your bet was deleted") ───────
// Unlike `notifications` (a global pool every user sees via a join), each row
// here belongs to exactly one user. Used for direct, personal alerts pushed
// over the "user-notifications" WS channel + shown on the header bell.
...
export const transactions = pgTable("transactions", {
  ...
  // True when this bet was auto-generated by the Cashout flow (a hedge placed
  // to flatten the user's exposure on the market) rather than a manual bet.
  isCashout: boolean("is_cashout").default(false).notNull(),
...
export const competitions = pgTable("competitions", {
  ...
  // Owner-pinned competitions surface in the site's top "drop header" nav under
  // a custom label the owner types when pinning. `pin_label` is that label and
  // is null when the competition isn't pinned. Pinning respects per-whitelabel
  // disables (a whitelabel that turned the competition off won't see the entry).
  is_pinned: boolean("is_pinned").default(false).notNull(),
  pin_label: varchar("pin_label", { length: 120 }),

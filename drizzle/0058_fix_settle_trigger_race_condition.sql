-- ─────────────────────────────────────────────────────────────────────────────
-- FIX: Settlement trigger race condition
--
-- The original settlement trigger (0051) did NOT lock the ledger_limit row
-- before reading + updating it. When two bets settle concurrently for the
-- same user, the second trigger reads stale values and overwrites the first
-- trigger's balance/exposure changes — causing intermittent incorrect ledger
-- updates ("sometimes updates, sometimes not").
--
-- Fix: Add FOR UPDATE to the ledger_limit SELECT, matching the bet-placement
-- trigger's serialization approach.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Recreate trigger (same definition, updated function)
